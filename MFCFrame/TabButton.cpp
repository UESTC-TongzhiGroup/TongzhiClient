...
{
	return CWnd::CreateEx(0,_T("TabButton"),lpszWindowName,dwStyle,rect,pParentWnd,nID);
}
...
void CTabButton::OnMouseHover(UINT nFlags, CPoint point)
{
	CWnd::OnMouseHover(nFlags, point);
}

void CTabButton::OnMouseLeave()
{
	CWnd::OnMouseLeave();
}
...
void CTabButton::OnPaint()
{
	...
	if(m_bIsSelected)
	{
		...
		rcItemRight.left=m_imgbtn.GetWidth()-(m_imgbtn.GetWidth()/4/2-1)+135;
	...
	else
	{
		CRect rcItemLeft = rt;
		rcItemLeft.right = m_imgbtn.GetWidth() / 4 / 2-1;
		m_imgbtn.Draw(dc.GetSafeHdc(), rcItemLeft, CRect(0, 0, m_imgbtn.GetWidth() / 4 / 2 - 1, m_imgbtn.GetHeight()));

		CRect rcItemCenter = rt;
		rcItemLeft.left = m_imgbtn.GetWidth() / 4 / 2 - 1;
		rcItemLeft.right = m_imgbtn.GetWidth() - (m_imgbtn.GetWidth() / 4 / 2 - 1);
		m_imgbtn.Draw(dc.GetSafeHdc(), rcItemCenter, CRect(m_imgbtn.GetWidth() / 4 / 2 - 1, 0, m_imgbtn.GetWidth() / 4 / 2, m_imgbtn.GetHeight()));

		CRect rcItemRight = rt;
		rcItemRight.left = m_imgbtn.GetWidth() - (m_imgbtn.GetWidth() / 4 / 2 - 1)+130;
		m_imgbtn.Draw(dc.GetSafeHdc(), rcItemRight, CRect(m_imgbtn.GetWidth()/4/2, 0, m_imgbtn.GetWidth() * 1 / 4, m_imgbtn.GetHeight()));
	}
...
void CTabButton::SetTabForButton( UINT uIDResource )
{
   if(!m_imgbtn.IsNull())
   {
	   m_imgbtn.Destroy();
...
BOOL CTabButton::OnEraseBkgnd(CDC* pDC)
{
	return TRUE;
}
